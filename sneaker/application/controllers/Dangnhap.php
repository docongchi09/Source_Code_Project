...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='MessiEsport - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        $todaylimit = date("Y-m-d", strtotime($todaylimit));
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'ankhangatat5@gmail.com';
            $config['smtp_pass']    = '020620Khang';
            ...
            $this->email->from('ankhangatat5@gmail.com', 'MessiEsport');
            $this->email->to($email);
            $this->email->subject('MessiEsport - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng MessiEsport, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
                Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            $this->email->send();
            $this->data['success']='Đăng ký thành công! Bạn đã là thành viên mới !';

        }  
        $this->data['title']='MessiEsport - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->subject('MessiEsport - Lấy lại mật khẩu');
        ...
        $this->data['title']='MessiEsport - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='MessiEsport - Cập nhật mật khẩu mới';
